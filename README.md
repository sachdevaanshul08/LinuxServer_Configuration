...
On our Amazon Lightsail account we have created our own Ubuntu Instanace and created and confifure a virtual machine to deploy of python enabled application.
...
#### 2. Update the installe packages
...
on using `ssh ubuntu@13.232.197.59 -p 2200 -i ~/Downloads/name_of_the_file.pem`
...
Swith from the existing user to the newly created user that is 'grader'
`su - grader`
...
* Make changes for the app in Google API Console accept the lightsail public IP, URL and redirect URIs. Edit the secret files on server to reflect the changes if any.
...
* Change the file references for client_secrets in the python files 
